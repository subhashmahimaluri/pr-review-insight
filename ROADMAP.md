...
| 7.3    | **Per-finding "Fix with AI" links in the PR comment**             | each 🆕 row gets a `Copy prompt` `<details>` block; one click in the GitHub UI, paste into Copilot Chat in the IDE — no integration surface at all                                                                                              |
